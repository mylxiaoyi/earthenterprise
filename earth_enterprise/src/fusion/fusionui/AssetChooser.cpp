...
class ChangeDirEvent : public QEvent {
 public:
...
class FolderItem : public QWidget{
 public:
  FolderItem(QWidget* parent, const gstAssetFolder& f);
...
FolderItem::FolderItem(QWidget* parent, const gstAssetFolder& f)
    : QWidget(parent), folder(f) {
...
class AssetItem : public QWidget{
 public:
  AssetItem(QWidget* parent, gstAssetHandle handle);
...
AssetItem::AssetItem(QWidget* parent, gstAssetHandle handle)
    : QWidget(parent), assetHandle(handle) {
...
AssetChooser::AssetChooser(QWidget* parent, AssetChooser::Mode m,
    ...
    : QDialog(parent), ui(new Ui_AssetChooserBase), mode_(m),
      type_(t), subtype_(st),
  ...
  if (mode_ == Save || mode_ == SaveAs) {
    this->ui->ok_btn->setText(SaveBtnText);
    setWindowTitle(SaveBtnText);
  } else {
    this->ui->ok_btn->setText(OpenBtnText);
    setWindowTitle(OpenBtnText);
  }
  ...
  this->ui->ok_btn->setEnabled(false);
...
bool AssetChooser::matchFilter(const gstAssetHandle handle) const {
  ...
  return false;//(a.PrettyName() == match_string_);
}
...
void AssetChooser::accept() {
  if (mode_ == Open) {
    ...
    /*AssetItem* assetItem = dynamic_cast<AssetItem*>(item);
    if (assetItem != NULL) {
      this->ui->nameEdit->setText(shortAssetName(assetItem->getAssetHandle()->getName()));
...
void AssetChooser::nameChanged(const QString& str) {
  /*if (str.isEmpty()) {
    ...
    this->ui->ok_btn->setEnabled(true);
    this->ui->ok_btn->setDefault(true);
  }*/
}
...
void AssetChooser::NewDirectory() {
  ...
  QString text("mylxiaoyi");
  if (ok && !text.isEmpty()) {
    QString newdir(current_folder_.fullPath() + "/" );//+ text);
    QString error;
    if (!theAssetManager->makeDir
        (AssetDefs::FilenameToAssetPath(newdir.toStdString().c_str()), error)) {
      QMessageBox::critical(this, tr("Error"),
