    ...
    flipBuf()
{
  ...
  if (pyrfile.fd() < 0) {
    throw khErrnoException(kh::tr("Unable to open %1").arg(QString(filename.c_str())));
  }
  if (!pyrio::IndexStorage::Header::Read(header_, pyrfile.fd())) {
    throw khException(kh::tr("Error reading %1").arg(QString(filename.c_str())));
  }
  switch (header_.compressMode) {
    ...
    case CompressJPEG:
      if (header_.componentType != khTypes::UInt8) {
        throw khException(kh::tr("Error reading %1: JPEG compression with non std::uint8_t pixel type").arg(QString(filename.c_str())));
      }
    ...
    case CompressLZ:
      ...
      break;
    case CompressDXT1:
      throw khException(kh::tr("Error reading %1: DXT1 decompression not supported").arg(QString(filename.c_str())));
      break;
    case CompressPNG:
      throw khException(kh::tr("Error reading %1: PNG decompression not supported").arg(QString(filename.c_str())));
      break;
  }
      ...
      runningOffset(0)
{
  if (compressMode == CompressJPEG &&
    ...
    throw khException(kh::tr("Error writing %1: JPEG compression with non std::uint8_t pixel type").arg(QString(filename.c_str())));
  }
  ...
  if (!pyrio::IndexStorage::Header::Write(pyrfile.fd(), header)) {
    throw khException(kh::tr("Error writing %1").arg(QString(filename.c_str())));
  }
