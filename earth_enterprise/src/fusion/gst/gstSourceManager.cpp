...
gstSharedSource gstSourceManager::GetSharedAssetSource(
  ...
  if (!found) {
    ...
    if (!asset) {
      throw khException(kh::tr("No such asset: '%1'").arg(QString::fromStdString(assetRef)));
    }
    ...
    if (ver) {
      ...
      throw khException(kh::tr("Asset '%1' doesn't have a good version")
                        .arg(QString::fromStdString(assetRef)));
    }
    ...
    sourceId(), source(), path(path_), key(key_)
{
  ...
  if (source->Open() != GST_OKAY) {
    throw khException(kh::tr("Unable to open '%1'").arg(QString::fromStdString(path)));
  }
