...
std::string gstValue::ValueAsCSV() const {
  switch (type_) {
    ...
    case gstTagString:
    case gstTagUnicode:
      return std::string(FormatAsCSV(qstring_->toStdString().c_str()));
  }
  ...
  switch (type_) {
    ...
    case gstTagUnicode:
      if (codec_) {
        ...
        *qstring_ = QString::fromStdString(ExtractString(buf, len, &need_codec_));
      }
      break;
  }
}
...
void gstValue::set(const QString& unicode) {
  switch (type_) {
    ...
    case gstTagUInt:
      ...
      break;
    case gstTagInt64:
      num_data_.i64Val = strtoll(unicode.toStdString().c_str(), NULL, 0);
      break;
    case gstTagUInt64:
      num_data_.ui64Val = strtoull(unicode.toStdString().c_str(), NULL, 0);
      break;
    case gstTagFloat:
    ...
    case gstTagUnicode:
      if (qstring_->isEmpty()) {
        *((char*)buf) = '\0';
        ...
        strcpy((char*)buf, qstring_->toStdString().c_str());
      }
      break;
  }
}
  ...
  switch (type_) {
    ...
    case gstTagUnicode:
      if (qstring_->isEmpty()) {
        ...
        return std::int64_t(strtoll(qstring_->toStdString().c_str(), NULL, 0));
      }
  }
  return 0;
}
  ...
  switch (type_) {
    ...
    case gstTagUnicode:
      if (qstring_->isEmpty()) {
        ...
        return static_cast<std::uint64_t>(strtoull(qstring_->toStdString().c_str(), NULL, 0));
      }
  }
  return 0;
}
...
QString gstValue::ValueAsUnicode() const {
  ...
  switch (type_) {
    ...
    case gstTagUInt:
      ...
      break;
    case gstTagInt64:
      string = QString::fromStdString(ValueAsString());
      break;
    case gstTagUInt64:
      string = QString::fromStdString(ValueAsString());
      break;
