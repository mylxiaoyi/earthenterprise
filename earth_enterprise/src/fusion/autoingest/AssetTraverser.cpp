...
void AssetTraverser::Traverse(void)
{
  ...
  if (!khDirExists(assetroot_)) {
    throw khException(kh::tr("%1 isn't a directory").arg(QString(assetroot_.c_str())));
  }
                                     ...
                                     const std::string &fname)
{
  ...
  if (!asset) {
    throw khException(kh::tr("Unable to load asset %1").arg(QString(assetref.c_str())));
  }
