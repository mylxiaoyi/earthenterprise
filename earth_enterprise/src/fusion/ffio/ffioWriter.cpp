...
ffio::Writer::OpenNextFile(void)
{
  ...
  if (fileHandle.fd() < 0) {
    throw khErrnoException(kh::tr("Unable to create %1").arg(QString(fname.c_str())));
  }
}
  ...
  if (!khWriteAll(fileHandle.fd(), &hdr, sizeof(hdr))) {
    throw khErrnoException
      (kh::tr("Failed to write ff tile header (lrc:%1,%2,%3) in %4")
       .arg(addr.level).arg(addr.row).arg(addr.col).arg(QString(outdir.c_str())));
  }
