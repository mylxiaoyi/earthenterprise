...
khPresenceMask::khPresenceMask(const std::string &filename)
  ...
  if (!khGetFileInfo(filename, fileSize, mtime)) {
    throw khErrnoException(
        kh::tr("Unable to get file size for %1").arg(QString(filename.c_str())));
  }
  ...
  if (header.presenceFormatVersion == 1) {
    ...
    throw khException(kh::tr
                      ...
                      arg(header.presenceFormatVersion).arg(QString(filename.c_str())));
  }
  ...
  if (filebuf == MAP_FAILED) {
    throw khErrnoException(kh::tr("Unable to mmap %1").arg(QString(filename.c_str())));
  }
    ...
    if (rec.levelNum >= NumFusionLevels) {
      throw khException
        (kh::tr("Corrupted level header (levelNum) %1: level %2").
         arg(QString(filename.c_str())).arg(rec.levelNum));
    }
