  ...
  if (khExists(pidfile)) {
    ...
    if (khReadSimpleFile(pidfile, pidstr)) {
      ...
      if (pid > 0) {
        ...
        throw khException(kh::tr("Unable to parse pid from %1").arg(QString(pidfile.c_str())));
      }
    } else {
      throw khException(kh::tr("Unable to read pidfile %1").arg(QString(pidfile.c_str())));
    }
  }
  return false;
}
